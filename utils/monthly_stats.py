...
def get_monthly_stats(data):
    ...
    monthly_stats = data.groupby(['Month', 'Month_Num', 'Year'])['Amount'].agg(['mean', 'count', 'sum']).round(2)
    ...
    for _, row in monthly_stats.iterrows():
        entry = {
            ...
            "total_amount": convert_numpy_type(row['sum']),
            "transaction_count": convert_numpy_type(row['count']),
            "average_amount": convert_numpy_type(row['mean']),
    ...
    monthly_analysis = {
        ...
        'peak_month_amount': convert_numpy_type(monthly_stats.loc[peak_month_idx, 'sum']),
        'lowest_month_amount': convert_numpy_type(monthly_stats.loc[lowest_month_idx, 'sum']),
