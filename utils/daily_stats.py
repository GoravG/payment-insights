...
def get_daily_stats(data):
    ...
    daily_stats = (
        data.groupby("Day_of_Week")["Amount"].agg(["mean", "count", "sum"]).round(2)
    )
    ...
    daily_patterns = {
        str(day): {
            ...
            "average_per_transaction": to_native(stats["mean"]),
            "average_daily_total": to_native(stats["daily_average"]),
        }
